...
%plotBeampatternsWithError Plot the mean beampattern with error bounds
...
% Show standard deviation as two lines around the mean, fill is not supported for polarplot in older matlab versions
polarplot(pax, theta, lower_db, '-', 'LineWidth', 0.5, 'Color', 'black');
polarplot(pax, theta, upper_db, '-', 'LineWidth', 0.5, 'Color', 'black');

% fill(pax, thetaAll, rAll, [0.7 0.7 0.7], 'FaceAlpha', 0.5, 'EdgeColor','none'); % Only works with newer matlab versions
